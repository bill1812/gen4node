...
*/
    ...
    */
    ...
    describe('npm start', function () {
      ...
      it('should respond to HTTP request', function (done) {
        request(this.app).get('/').expect(200, /<title>Express<\/title>/, done);
      });

      it('should generate a 404', function (done) {
        request(this.app).get('/does_not_exist').expect(404, /<h1>Not Found<\/h1>/, done);
      });
    });
  ...
  describe('--css <engine>', function () {
    ...
    //  assert.notEqual(ctx.files.indexOf('public/stylesheets/style.less'), -1, 'should have style.less file');
        assert.notEqual(ctx.files.indexOf('less/style.less'), -1, 'should have style.less file');
      });
      ...
      //  request(this.app).get('/stylesheets/style.css').expect(200, /sans-serif/, done);
          request(this.app).get('/css/style.css').expect(200, /sans-serif/, done);
        });
      });
    }); // end of less 
    ...
    //  assert.notEqual(ctx.files.indexOf('public/stylesheets/style.styl'), -1, 'should have style.styl file');
        assert.notEqual(ctx.files.indexOf('styl/style.styl'), -1, 'should have style.styl file');
      });
  ...
  describe('--no-view', function () {
    ...
    describe('npm start', function () {
      ...
      it('should generate a 404', function (done) {
        request(this.app).get('/does_not_exist').expect(404, /Cannot GET \/does_not_exist/, done);
      });
    });
  }); // end of --no-view
...
function runRaw(dir, args, callback) {
  ...
  var stderr = '';
  var stdout = '';
  var child  = spawn(binp, argv, { cwd: dir });
  child.stdout.setEncoding('utf8');
  child.stdout.on('data', function ondata(str) { stdout += str; });
  child.stderr.setEncoding('utf8');
  child.stderr.on('data', function ondata(str) { stderr += str; });
